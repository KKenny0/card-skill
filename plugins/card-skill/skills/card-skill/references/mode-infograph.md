...
| 每个区块背景相同 | 有真实分组需要才增加底色，优先用间距和对齐。 |
| 三列等宽 | 同级内容可以等宽；只有真实主次才使用不同宽度。 |
| 标题居中 | 检查阅读起点和顺序是否清楚，不为造型强行改位置。 |
| 每张图都有"公式条带" | 这不是必须的。有些思想没有公式。 |
| 弹点色用了 3 处以上 | 回到 2 处。弹点是子弹。 |
| 没有一个元素超过 100px | 只检查重点是否清楚，不为达标放大元素。 |
| 所有文字大小相近 | 同级内容可以同字号；主次不清时再调整字重、间距或字号。 |
| 正文字号小于 36px | 手机上会不可读。最小正文 36px，最小标注 24px。 |
| 区块之间间距都一样 | 同级分组可用相同间距；不同层级才需要区分。 |
...
写入操作系统临时目录：`<system_temp>/card_infograph_{name}.html`
...
```bash
node assets/capture4k.js <system_temp>/card_infograph_{name}.html ~/Downloads/{name}.png 1080 800 2 fullpage
```
