...
按 mode 分配两套中文字体。等粗宋管阅读类（干净温润），打字机体管表现类（洇墨质感）。
...
| 漫画 comic | 香萃打字机体 | 香萃打字机体 | 黑白戏剧 + 断连 = 分镜感 |
...
### 排版
- **禁止 Inter 字体**：统一使用 DM Serif Display（标题）+ DM Sans（正文）+ Noto Serif SC（中文）
...
wjy-mockup 的输出应该像**印刷品**，不是网页截图。暗色系和亮色系的策略不同：
...
**饱和度压制 — 像 CMYK 印刷**：
- 品牌强调色使用时降饱和度 10-20%。屏幕上的 #5e6ad2 印出来应该是 #6872be 的感觉
...
### 玻璃态（Glassmorphism）
仅在品牌设计系统明确要求时使用，默认不使用。wjy-mockup 偏纸质印刷感，回避数字 UI 隐喻。
