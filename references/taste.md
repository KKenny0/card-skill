...
card-skill 的默认视觉系统不是主题市场，而是一套安静的纸面系统。所有品牌设计系统只提供气质偏向，不允许重写底层版面纪律。

**统一骨架：**
- 页面像完成的纸面：暖纸底、深墨文字、极少装饰。
- 层级靠字号、留白、位置和细线，不靠大阴影、发光、炫彩块。
- 卡片只在内容真的需要分组时出现，默认用留白和 hairline 分隔。
- 品牌色只作为低面积标记，不能成为背景主色或大面积填充。
- 同一张图内最多一个彩色强调色；暗色图也是深色卡纸，不是发光 UI。

**品牌收敛规则：**
- Apple 保留产品留白感，Stripe 保留数据秩序，Claude 保留人文温度，Linear 保留精密感，Vercel 保留黑白工程感。
- 这些差异只能体现在 accent、surface 温度、分隔节奏和少量语义组件上。
- 字体、正文尺度、纸面材质、圆角上限、阴影上限、出厂检查全局统一。
- ljg-card 现在是内容预处理和卡片隐喻来源之一，不再是默认视觉母体。
...
### 字体由 mode 固定决定，不受品牌设计系统影响

品牌设计系统只影响气质（muted accent、surface 温度、留白节奏），不影响字体、正文尺度、卡片纪律和纸面材质。字体由 mode 唯一决定。
...
品牌 accent 的视觉强度决定弹点计数方式。Quiet Paper 默认将所有品牌色降噪为低饱和墨色，彩色面积应明显小于内容面积：
...
card-skill 的输出应该像**印刷品**，不是网页截图。暗色系和亮色系的策略不同：
...
**饱和度压制 — 像 CMYK 印刷**：
- 品牌强调色使用时降饱和度 10-20%。屏幕上的高饱和色应降成更像纸墨的低饱和色
...
### 玻璃态（Glassmorphism）
仅在品牌设计系统明确要求时使用，默认不使用。card-skill 偏纸质印刷感，回避数字 UI 隐喻。
