...
写入：
- 单张：`/tmp/cast_comic_{name}.html`
- 多张：`/tmp/cast_comic_{name}_{N}.html`（N = 01, 02, ...）
...
单张：
```bash
node assets/capture.js /tmp/cast_comic_{name}.html ~/Downloads/{name}.png 1080 800 fullpage
```

多张：逐张截图，文件名带序号
```bash
node assets/capture.js /tmp/cast_comic_{name}_01.html ~/Downloads/{name}_01.png 1080 800 fullpage
node assets/capture.js /tmp/cast_comic_{name}_02.html ~/Downloads/{name}_02.png 1080 800 fullpage
# ... 逐张执行
```
